...
df$ibutton <- as.factor(df$ibutton)
...
p2<- ggplot(data =subset(br, ibutton == i), aes(date.time, fish.depth))+
  geom_point(aes(colour = hour), size = 2)+
  scale_color_viridis(option = "A")+
  facet_wrap(~ibutton)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
p3<- ggplot(data =subset(br, ibutton == i), aes(date.time, fish.depth))+
  geom_point(aes(colour = hour), size = 2)+
  scale_color_viridis(option = "A")+
  facet_wrap(~ibutton)+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
...
figure <- ggarrange(p1, p2, p3, bp + font("x.text", size = 10),
                    ncol = 1, nrow = 3)
}

p<- grid.arrange(p1, p2, p3, nrow =3, top =textGrob("Blue Ruin netpen fish *(i)", gp=gpar(fontsize = 20)))

ggsave(p, file=paste0("figures/ibutton.figures/grid.plot.fish.temp.depth.do/ibutton.14.do.temp.depth.plot.png"), width = 50, height = 40, units = "cm") 

array <- read.csv("data/temp.do.data/do.temp.combined/norwood.mouth.temp.do.combined.csv")
...
#ggsave(pp, file=paste0("figures/ibutton.figures/norwood.netpen/temp.do.depth.time.png"), width = 50, height = 30, units = "cm") 
