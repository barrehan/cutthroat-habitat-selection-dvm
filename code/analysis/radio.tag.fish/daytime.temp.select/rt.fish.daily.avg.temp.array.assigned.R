...
for(i in tags){
  ...
  fig<-annotate_figure(figure,
                       ...
                       fig.lab = "Blue Ruin radio tag fish temperature vs available")
  #ggsave(ibutton.plot, file=paste0("figures/ibutton.figures/br.netpen1/br.netpen1.ibutton.", i,".png"), width = 20, height = 10, units = "cm")
  ggsave(fig, file = paste0("results/figures/radio.tag.figures/temp.select.daytime/daytime.temp.assigned.array/temp.select.daytime/br.rt.", i,".temp.select.daytime.jpg"), width = 12, height = 8, units = "in")
...
case.1 <- case[case$case == 1,]
ggplot(data = case, aes(x= temperature, y = depth))+
  geom_line()+
  geom_point(data = case.1, aes(x =temperature, y = depth))

#NEXT
#calculate frequency that case = 1 occurs at temp != to min temp or max temp
#in array
