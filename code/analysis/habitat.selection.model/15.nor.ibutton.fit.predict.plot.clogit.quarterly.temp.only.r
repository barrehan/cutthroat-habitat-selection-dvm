...
pred.vals.quart1.vary.temp <- data.frame(standardized.temp = seq(min(quart1$standardized.temp, na.rm = T),
...
pred.vals.quart2.vary.temp <- data.frame(standardized.temp = seq(min(quart2$standardized.temp, na.rm = T),
...
pred.vals.quart3.vary.temp <- data.frame(standardized.temp = seq(min(quart3$standardized.temp, na.rm = T),
...
pred.vals.quart4.vary.temp <- data.frame(standardized.temp = seq(min(quart4$standardized.temp, na.rm = T),
...
plot.temp <-ggplot(preds.temp, aes(x=standardized.temp, y=fit, color=time, fill=time, group=time)) +
  ...
  labs(title = "Norwood fish temperature selection",
       subtitle = "DO excluded from model")+
  xlab("Standardized temperature (°C)") + ylab("Relative Probability of Selection")+
  theme(legend.title = element_blank()) 


ggsave(plot.temp, filename = paste("results/figures/hab.select.mod.figures/norwood.quarterly.temp.selection.do.excluded.png"), width = 12, height = 8, units = "cm")
