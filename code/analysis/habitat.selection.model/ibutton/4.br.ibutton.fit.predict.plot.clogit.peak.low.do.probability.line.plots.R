...
pred.vals.noon.avgDO <- data.frame(standardized.do = 0.3,
...
m <-ggplot(preds.noon.avgDO, aes(x=standardized.temp, y=fit)) +
  ...
  labs(title = "Blue Ruin temperature selection",
       subtitle = "Early evening; 17:00 - 19:00 (4.2mg/L)")+
...
pred.vals.night.lowDO <- data.frame(standardized.do = -0.9,
...
e <-ggplot(preds.night.lowDO, aes(x=standardized.temp, y=fit)) +
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        text = element_text(size = 15, family = "serif"))+
  labs(title = "Blue Ruin temperature selection",
       subtitle = "Early morning; 05:00 - 07:00 (2.6mg/L)")+
  xlab("Standardized temperature (\u00B0C)") + ylab("Relative probability of selection")+
  theme(legend.title = element_blank()) 
...
ggsave(f, filename = paste("results/figures/hab.select.mod.figures/line.plots/br.2hr.hypoDOavg.png"), width = 23, height = 10, units = "cm")
