...
pred.vals.quart1.vary.do <- data.frame(standardized.do = seq(min(quart1$standardized.do, na.rm = T),
...
pred.vals.quart2.vary.do <- data.frame(standardized.do = seq(min(quart2$standardized.do, na.rm = T),
...
pred.vals.quart3.vary.do <- data.frame(standardized.do = seq(min(quart3$standardized.do, na.rm = T),
                                                             max(quart3$standardized.do, na.rm = T), 
                                                             0.1),
                                       stratID = 61)
...
pred.vals.quart4.vary.do <- data.frame(standardized.do = seq(min(quart4$standardized.do, na.rm = T),
...
plot.do <- ggplot(preds.do, aes(x=standardized.do, y=fit, color=time, fill=time, group=time)) +
  ...
  labs(title = "Norwood fish DO selection",
       subtitle = "Temp excluded from model")+
  xlab("Standardized dissolved oxygen (mg/L)") + ylab("Relative Probability of Selection")+
  theme(legend.title = element_blank()) 

ggsave(plot.do, filename = paste("results/figures/hab.select.mod.figures/norwood.quarterly.do.selection.temp.excluded.png"), width = 16, height = 8, units = "cm")
