...
br.ib <- br.ib[br.ib$date.time >= "2021-08-01 00:00:00" & br.ib$date.time < "2021-08-05 00:00:00",]
...
ggsave("results/figures/blue-ruin-average-effect-plot-0801-0804.png", plot = all, width = 8, height = 9, dpi = 300)                    
...
# High-DO window
do_br_high   <- calc_avg_effect(gam.high, "standardized.do", mean.do, sd.do)
temp_br_high <- calc_avg_effect(gam.high, "standardized.temp", mean.t, sd.t)

# Low-DO window
do_br_low    <- calc_avg_effect(gam.low, "standardized.do", mean.do, sd.do)
temp_br_low  <- calc_avg_effect(gam.low, "standardized.temp", mean.t, sd.t)

# Visualize data

pk_tbl <- dplyr::bind_rows(
  dplyr::mutate(as.data.frame(do_br_high$peak),  Panel = "DO-maxima: DO"),
  dplyr::mutate(as.data.frame(temp_br_high$peak),Panel = "DO-maxima: Temp"),
  dplyr::mutate(as.data.frame(do_br_low$peak),   Panel = "DO-minima: DO"),
  dplyr::mutate(as.data.frame(temp_br_low$peak), Panel = "DO-minima: Temp")
) |>
...
write.csv(pk_tbl, "results/tables/blue-ruin-gam-peak-selection-95CI-0801-0804.csv", row.names = FALSE)
...
# Optional: save to CSV
dir.create("results/tables", showWarnings = FALSE, recursive = TRUE)
write.csv(aic_table, "results/tables/blue-ruin-gam-aic-comparison-0801-0805.csv", row.names = FALSE)
